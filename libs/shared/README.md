...
`UserIdDto` validates owner lookup IDs; `OwnerSummary`, `LaptopDetails`, and
`LaptopWithOwner` describe the aggregation response, including its partial-data
flag and owner status. Changing a contract here updates all importing apps;
build and verify the services together after a change.
